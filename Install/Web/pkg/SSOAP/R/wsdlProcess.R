         ...
         addSoapHeader = FALSE, verbose = FALSE, force = FALSE)  
{
 ...
 for(i in operations) {
   ...
   funs[[convertToSName(i@name)]] = createOperationDefinition(i, server, def@types, env, nameSpaces, addSoapHeader)
 }
          ...
          .elementFormQualified = getElementFormQualified(.types))
{
...
#if(.operation@name == "GetRecordMol") browser() 
 ...
 txt = paste("function(",
              paste(c(sapply(names(.operation@parameters), convertToSName),
                      paste("server = .defaultServer, .convert = ", converter, ", .opts = list(...), ...")),
                    collapse=",\n\t "),
...
function(name,  server, operations, types, nameSpaces = NA, obj = new("SOAPServerDescription"))
{
 obj@name = name
 obj@server = server
         ...
         port = 1L)
{
 ...
 types = processTypes(root, verbose = verbose)
...
function(root, verbose = FALSE)
{
   ...
   wImports = xpathApply(root, "//w:import",  namespaces = c(w = "http://schemas.xmlsoap.org/wsdl/"),
                              processWSDLImport, baseURL = baseURL)
   if(length(wImports)) {
...
function(node, doc = NULL, typeDefinitions = list())
{
 ...
 types = doc[xmlSApply(doc, function(x) xmlName(x) == "portType" && xmlGetAttr(x, "name") == type)][[1]]
...
#                                       resolve(el, typeDefinitions)@name
     ...
     args = args[ ! sapply(args, checkIsEmptyParameter, types = typeDefinitions) ]
...
function(type, types)
{
   tt = resolve(type, types)
   if(is(tt, "ClassDefinition") && length(tt@slotTypes) == 0)
      return(TRUE)
